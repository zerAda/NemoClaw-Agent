...
class Scraper:
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=20.0) as client:
                    ...
                    if resp.status_code == 429:
                        self.logger.warning(f"FT Rate Limit (429), attempt {attempt+1}")
                ...
                self.logger.error(f"FT failure (Attempt {attempt+1}): {type(e).__name__}")
                await asyncio.sleep(1)
        
        await self.alerter.send_alert("France Travail", "All retries failed")
        return []
    ...
    async def search_wttj_jobs(self, query: str, location: str = "France") -> List[JobListing]:
        sel = self.selectors.get("wttj", {})
        if not sel: return []
    ...
    async def get_job_description(self, job_url: str, source: str) -> str:
        """Unified JD extraction logic with platform-specific selector routing."""
        sel = self.selectors.get(source, self.selectors.get("linkedin"))
        ...
        async with self as managed:
            ...
            try:
                ...
                selector = sel.get("job_description")
                if not selector:
                    self.logger.warning(f"No description selector for {source}")
                    return ""
                    
                jd_elem = await page.query_selector(selector)
                return (await jd_elem.inner_text()).strip() if jd_elem else ""
            except Exception as e:
                self.logger.error(f"JD Extraction Fail ({source}): {type(e).__name__}")
                return ""
            finally:
                await page.close()
